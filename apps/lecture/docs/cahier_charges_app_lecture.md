...
### 2.3 Structure des 8 écrans d'apprentissage par phonème

Chaque phonème/graphème suit une séquence standardisée de 8 activités progressives :
...
**Écran 8 : "J'écris des phrases sous la dictée"**
- **Objectif** : Encodage de phrases complètes et automatisation orthographique
- **Activités** : Dictée audio, saisie/écriture de phrases, auto-correction guidée
- **Critère de réussite** : 7/10 phrases correctement écrites
...
### 3.0 Contexte monorepo et migration

Le projet d'initiation a la lecture est integre dans un monorepo multi-applications.

- **Monorepo cible** : enaa-francais-app
- **Positionnement** : application Next.js dans le monorepo (package/app)
- **Migration** : conversion du code Vite actuel vers Next.js App Router
- **Repertoire source** : /home/ceredis/mon-premier-cahier
- **Repertoire cible** : /home/ceredis/enaa-francais-app
- **Contraintes** : navigation data-driven selon le curriculum, separation stricte packages/apps
...
#### Contexte monorepo
- **Monorepo cible** : enaa-francais-app
- **Positionnement** : application Next.js integree comme package/app du monorepo
- **Navigation** : data-driven (curriculum) selon l'architecture monorepo CEREDIS
- **Migration** : conversion du projet Vite actuel vers Next.js App Router

#### Backend & Infrastructure
- **API** : Next.js API Routes + tRPC
- **Base de données** : Supabase PostgreSQL (auto-heberge, Coolify)
- **Authentification** : Supabase Auth (auto-heberge)
- **Storage** : Supabase Storage (medias)
- **Hosting** : VPS via Coolify
- **Analytics** : LRS Ralph (xAPI) + Grafana + Superset

#### Services transverses (multi-monorepos)
- **Supabase partage** : enaa-francais-app, enaa-maths-app, enaa-sciences-app
- **LRS** : Ralph sur https://lrs.ceredis.net
- **Observabilite** : Grafana sur https://analytics.ceredis.net
- **BI** : Superset sur https://reports.ceredis.net
...
graph TB
    ...
    subgraph "Analytics - LRS Ralph + BI"
      U[Ralph LRS<br/>xAPI Endpoint]
      V[Grafana]
      W[Superset]
    end
...
## 5. Intégration LRS Ralph et tracking xAPI

### 5.1 Configuration LRS Ralph via Coolify

#### Architecture xAPI
- **LRS Ralph** : https://lrs.ceredis.net
- **Observabilite** : Grafana (https://analytics.ceredis.net)
- **BI / reporting** : Superset (https://reports.ceredis.net)
- **Deploiement** : VPS via Coolify (services containerises)
- **Flux** : Frontend/Backend → LRS Ralph (xAPI) → dashboards Grafana/Superset
...
interface ProjectPhases {
  ...
  phase2_complete: {
    ...
    scope: [
      ...
      'Reconnaissance vocale et écriture tactile',
      'Dashboard enseignants',
      'Analyse cognitive pour la recherche'
    ],
    budget: '240k€'
  };
...
interface OfflineStrategy {
  ...
  offlineExercises: '20 activités minimum'
}
```
...
interface SuccessCriteria {
  fonctionnel: {
    parcoursComplet: '8 écrans phonème → phrase'
  };
  technique: {
    perf: 'LCP < 2.5s'
  };
  pedagogique: {
    acquisitionPhonemes: '80% réussite sur 5 phonèmes'
  };
}
```
