...
export const RealTimeFeedback: React.FC<RealTimeFeedbackProps> = ({
  ...
  const getBackgroundColor = (quality: string) => {
    switch (quality) {
      case 'excellent': return 'bg-green-100 border-green-400';
      case 'good': return 'bg-blue-100 border-blue-400';
      case 'warning': return 'bg-yellow-100 border-yellow-400';
      case 'needs_improvement': return 'bg-red-100 border-red-400';
      default: return 'bg-gray-100 border-gray-400';
    }
  };

  return (
    <div className={`absolute top-4 right-4 px-4 py-2 rounded-lg border-2 ${getBackgroundColor(strokeQuality.quality)}`}>
      <span className="font-medium">{strokeQuality.message}</span>
    </div>
  );
};
```
...
function generateMockPoints(letter: string): Point[] {
  ...
  for (let i = 0; i < 20; i++) {
    points.push({ x: 50 + i, y: 50 + Math.sin(i / 3) * 10, timestamp: startTime + i * 10 });
  }
...
function generateSmoothPoints(): Point[] {
  ...
  for (let i = 0; i < 15; i++) {
    points.push({ x: 30 + i, y: 40 + i * 0.5, timestamp: startTime + i * 15 });
  }
...
function generateFastPoints(): Point[] {
  ...
  for (let i = 0; i < 30; i++) {
    points.push({ x: 60 + i, y: 30 + i * 2, timestamp: startTime + i * 5 });
  }
...
function generatePoorQualityPoints(): Point[] {
  ...
  for (let i = 0; i < 25; i++) {
    points.push({ x: 20 + Math.random() * 50, y: 20 + Math.random() * 50, timestamp: startTime + i * 20 });
  }
...
export class CanvasOptimizer {
  ...
  throttledDraw(ctx: CanvasRenderingContext2D, point: Point) {
    ...
    if (this.rafId === null) {
      this.rafId = requestAnimationFrame(() => this.flushPendingPoints(ctx));
    }
  }
